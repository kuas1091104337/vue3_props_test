...
app.directive("price", {
  ...
  mounted(el, num) {
    el.innerHTML = numPrice(num.value);
  },
...
app.directive("model-save", {
  mounted(el, binding) {
    console.log(binding);
    // binding = Proxy, binding.value = v-model-save 代入的參數
    // 定義 3 個儲存的 function
    ...
    const setInputValue = (value) => (el.value = value),
      ...
      setLocalStorage = (value) =>
        localStorage.setItem(`form-${el.className}`, value);
  ...
  mounted(el, binding) {
    ...
    el.addEventListener("click", () => {
      ...
      const distance = 1000,
        ...
        srcollTime = Math.round(
          Math.abs(currentScrollTop - targetScrollTop) * rate
        );
      srcollTime < minSpeed
        ? minSpeed
        : srcollTime > maxSpeed
...
app.directive("scroll-active", {
  mounted(el, binding) {
    ...
    const { fixHeadName, addClass, targetName } = binding.value,
      ...
      minSpeed = 300,
      maxSpeed = 600,
      rate = 300 / 1000, // speed/distance 300ms走1000px(速率)
          ...
          step = (currentTime) => {
            ...
            const progressTime = currentTime - startTime, // 已經花費的時間
              moveDistance = Math.abs(
                progressTime * ((targetScrollTop - currentScrollTop) / moveTime)
              ); // 當前時間點須要移動的距離
            // 目標位置低於當前位置滑動算式 Math.min( + )                                // 目標位置高於當前位置滑動算式 Math.max( - )
            window.scrollTo(
              0,
              currentScrollTop < targetScrollTop
                ? Math.min(currentScrollTop + moveDistance, targetScrollTop)
                : Math.max(currentScrollTop - moveDistance, targetScrollTop)
            );
            if (progressTime < moveTime) {
              // 已經花費的時間 小於 滑動需要的總時間 - 遞迴
      ...
      windowScrollHandle = () => {
        ...
        timer = setTimeout(() => {
          ...
          if (offsetTop < scrollMiddle && offsetBottom > scrollMiddle) {
            if (
              el.classList.contains(addClass) &&
              target.classList.contains(addClass)
            )
              return;
            el.classList.add(addClass);
            target.classList.add(addClass);
